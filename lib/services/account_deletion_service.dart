...
class AccountDeletionService{
  ...
  Future<void> deleteCurrentUser(FinanceStore store)async{
    if(client.auth.currentUser==null)throw StateError('No hay una sesión autenticada.');
    final r=await client.functions.invoke('delete-account');
    if(r.status<200||r.status>=300)throw StateError('El servidor no pudo eliminar la cuenta (${r.status}). Los datos locales se conservaron para evitar pérdida parcial.');
