...
class FinancialAlertService {
  List<FinancialAlert> build(FinanceStore store, {DateTime? now, AlertPreferences preferences=const AlertPreferences()}) {
    ...
    if(preferences.budgets) if(preferences.budgets) for (final b in store.budgets) {
    ...
    if(preferences.recurring) if(preferences.recurring) for (final r in store.recurring.where((x)=>x.active)) {
    ...
    if(preferences.debts) if(preferences.debts) for (final d in store.debts) {
  ...
  DateTime _nextDayOfMonth(DateTime n,int day){DateTime candidate=DateTime(n.year,n.month,day.clamp(1,DateTime(n.year,n.month+1,0).day));if(candidate.isBefore(DateTime(n.year,n.month,n.day))) {final max=DateTime(n.year,n.month+2,0).day;candidate=DateTime(n.year,n.month+1,day.clamp(1,max));}return candidate;}
