...
class SecurityService {
  ...
  Future<PinCheckResult> verifyPin(String pin) async {
    ...
    if (salt != null && stored != null) {
      ...
      final legacySalt = p.getString(_saltKey),
          legacyHash = p.getString(_hashKey);
  ...
  Future<Duration> remainingLockout() async {
    final p = await SharedPreferences.getInstance();
    final ms =
        (p.getInt(_lockUntilKey) ?? 0) - DateTime.now().millisecondsSinceEpoch;
  ...
  Future<void> _resetAttempts(SharedPreferences p) async {
    await p.remove(_attemptsKey);
    await p.remove(_lockUntilKey);
  }
}
