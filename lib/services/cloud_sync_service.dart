...
class CloudSyncService {
  ...
  String _cloudId(String kind, String local) => _isUuid(local)
      ? local
      : const Uuid().v5(Namespace.url.value, 'finanzia:$uid:$kind:$local');
  String _entityCloudId(ChangeEvent e) =>
      e.entity == 'profile' ? uid : _cloudId(e.entity, e.entityId);
  ...
  Future<Set<String>> _remoteChangedKeys(
    DateTime since,
    String deviceId,
  ) async {
    final rows = await retryPolicy.run(
      () => client
          .from('sync_changes')
          .select('entity,entity_id,device_id')
          .eq('user_id', uid)
          .gt('changed_at', since.toUtc().toIso8601String())
          .order('changed_at'),
    );
    return rows
        .where((r) => r['device_id'] != deviceId)
        .map<String>((r) => '${r['entity']}:${r['entity_id']}')
        .toSet();
  }
  ...
  Future<void> _pruneChangeLog() async {
    ...
    await client
        ...
        .delete()
        .eq('user_id', uid)
        .lt('changed_at', cutoff.toIso8601String());
  }

  Future<SyncOutcome> smartSync(FinanceStore s) async {
    final remote = await cloudUpdatedAt();
    if (remote == null) {
      if (await _hasCloudData()) return SyncOutcome.conflict;
      await upload(s);
    ...
    final remoteChanged = remote.isAfter(
      last.add(const Duration(milliseconds: 1)),
    );
    if (localChanged && remoteChanged) {
      final remoteKeys = await _remoteChangedKeys(last, s.deviceId);
      if (remoteKeys.isEmpty ||
          remoteKeys.any((k) => k.startsWith('snapshot:')))
        return SyncOutcome.conflict;
      final localKeys = localPending
          .map((e) => '${e.entity}:${_entityCloudId(e)}')
          .toSet();
      if (remoteKeys.intersection(localKeys).isNotEmpty)
        return SyncOutcome.conflict;
  ...
  Future<void> uploadIncremental(FinanceStore s) async {
          ...
          pending
              .map(
                (e) => {
                  'user_id': uid,
                  'entity': e.entity,
                  'entity_id': _entityCloudId(e),
                  'action': e.action,
                  'device_id': s.deviceId,
                  'changed_at': e.changedAt.toUtc().toIso8601String(),
                },
              )
              .toList(),
        );
    ...
    await _pruneChangeLog();
  }

  Future<void> _applyChange(ChangeEvent e) async {
    final p = e.payload;
    if (e.entity == 'profile') {
      if (e.action != 'delete' && p != null)
    ...
    if (e.entity == 'account') {
      ...
      if (e.action == 'delete') {
        await client.from('accounts').delete().eq('user_id', uid).eq('id', id);
        return;
      }
      if (p != null)
        await client.from('accounts').upsert({
          ...
          'due_day': p['dueDay'],
          'updated_at': e.changedAt.toIso8601String(),
        });
      return;
    }
    if (e.entity == 'transaction') {
      ...
      if (p != null)
        await client.from('transactions').upsert({
          ...
          'account_id': _cloudId('account', p['accountId']),
          'destination_account_id': p['destinationAccountId'] == null
              ? null
              : _cloudId('account', p['destinationAccountId']),
          'occurred_at': DateTime.parse(p['date']).toUtc().toIso8601String(),
          'updated_at': e.changedAt.toIso8601String(),
        });
      return;
    }
    if (e.entity == 'budget') {
      ...
      if (e.action == 'delete') {
        await client.from('budgets').delete().eq('user_id', uid).eq('id', id);
        return;
      }
      if (p != null)
    ...
    if (e.entity == 'goal') {
      ...
      if (e.action == 'delete') {
        await client.from('goals').delete().eq('user_id', uid).eq('id', id);
        return;
      }
      if (p != null)
    ...
    if (e.entity == 'debt') {
      ...
      if (e.action == 'delete') {
        await client.from('debts').delete().eq('user_id', uid).eq('id', id);
        return;
      }
      if (p != null)
        await client.from('debts').upsert({
          ...
          'balance': p['balance'],
          'apr': p['apr'],
          'minimum_payment': p['minimumPayment'],
          'due_date': p['dueDate'] == null
              ? null
              : DateTime.parse(p['dueDate']).toIso8601String().split('T').first,
          'updated_at': e.changedAt.toIso8601String(),
        });
      return;
    }
    if (e.entity == 'recurring') {
      ...
      if (p != null)
        await client.from('recurring_payments').upsert({
          'id': id,
          'user_id': uid,
          'name': p['name'],
          'category': p['category'],
  ...
  Future<void> upload(FinanceStore s) async {
    final accountMap = {
      for (final a in s.accounts) a.id: _cloudId('account', a.id),
    };
                ...
                .map(
                  (t) => {
                    ...
                    'destination_account_id': t.destinationAccountId == null
                        ? null
                        : accountMap[t.destinationAccountId],
      ...
      await client
          ...
          .upsert(
            s.budgets
                .map(
                  (x) => {
                    'id': _cloudId('budget', x.id),
                    'user_id': uid,
                    'category': x.category,
                    'monthly_limit': x.limit,
                    'updated_at': now,
                  },
                )
                .toList(),
      ...
      await client
          ...
          .upsert(
            s.goals
                .map(
                  (x) => {
                    'id': _cloudId('goal', x.id),
                    'user_id': uid,
                    'name': x.name,
                    'target': x.target,
                    'current_amount': x.current,
      ...
      await client
          ...
          .upsert(
            s.debts
                .map(
                  (x) => {
                    'id': _cloudId('debt', x.id),
                    'user_id': uid,
                    'name': x.name,
                    'balance': x.balance,
                    'apr': x.apr,
                    'minimum_payment': x.minimumPayment,
                    'due_date': x.dueDate?.toIso8601String().split('T').first,
                    'updated_at': now,
                  },
                )
                .toList(),
      ...
      await client
          ...
          .upsert(
            s.recurring
                .map(
                  (x) => {
                    'id': _cloudId('recurring', x.id),
                    'user_id': uid,
                    'name': x.name,
                    'category': x.category,
                    'account_id': accountMap[x.accountId],
                    'amount': x.amount,
    ...
    await client.from('sync_changes').insert({
      'user_id': uid,
      'entity': 'snapshot',
      'entity_id': uid,
      'action': 'replace',
    ...
    final remote = await cloudUpdatedAt() ?? at;
    await s.markSynced(remote);
    await _pruneChangeLog();
  }
      ...
      accounts: aa
          .map<FinanceAccount>(
            (a) => FinanceAccount(
              id: a['id'],
              name: a['name'],
              type: AccountType.values.byName(a['type']),
              openingBalance: (a['opening_balance'] as num).toDouble(),
              creditLimit: (a['credit_limit'] as num?)?.toDouble(),
              statementDay: (a['statement_day'] as num?)?.toInt(),
              dueDay: (a['due_day'] as num?)?.toInt(),
            ),
          )
          .toList(),
      transactions: tt
      ...
      budgets: bb
          .map<Budget>(
            (x) => Budget(
              id: x['id'],
              category: x['category'],
              limit: (x['monthly_limit'] as num).toDouble(),
            ),
          )
          .toList(),
      goals: gg
          .map<SavingsGoal>(
            (x) => SavingsGoal(
              id: x['id'],
              name: x['name'],
              target: (x['target'] as num).toDouble(),
              current: (x['current_amount'] as num).toDouble(),
              targetDate: x['target_date'] == null
                  ? null
                  : DateTime.parse(x['target_date']),
            ),
          )
          .toList(),
      debts: dd
          .map<Debt>(
            (x) => Debt(
              id: x['id'],
              name: x['name'],
              balance: (x['balance'] as num).toDouble(),
              apr: (x['apr'] as num).toDouble(),
              minimumPayment: (x['minimum_payment'] as num).toDouble(),
              dueDate: x['due_date'] == null
                  ? null
                  : DateTime.parse(x['due_date']),
            ),
          )
          .toList(),
      recurring: rr
          .map<RecurringPayment>(
            (x) => RecurringPayment(
              ...
              dayOfMonth: x['day_of_month'],
              isExpense: x['is_expense'],
              active: x['active'],
            ),
          )
          .toList(),
