...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  String get label => switch (range) {
    ReportRange.month => 'Este mes',
    ReportRange.threeMonths => '3 meses',
    ReportRange.year => 'Este año',
    ReportRange.all => 'Todo',
  };

  @override
  Widget build(BuildContext c) {
    final tx = widget.store.transactions
        .where((x) => !x.date.isBefore(start))
        .toList();
    final inc = tx
        ...
        children: [
          Row(
            children: [
              ...
              PopupMenuButton<ReportRange>(
                ...
                itemBuilder: (_) => ReportRange.values
                    .map(
                      (v) => PopupMenuItem(
                        value: v,
                        child: Text(switch (v) {
                          ReportRange.month => 'Este mes',
                          ReportRange.threeMonths => 'Últimos 3 meses',
                          ReportRange.year => 'Este año',
                          ReportRange.all => 'Todo el historial',
                        }),
                      ),
                    )
                    .toList(),
          ...
          Wrap(
            ...
            children: [
              ...
              _m('Gastos', exp),
              _m('Ahorro', inc - exp),
              _m('Patrimonio neto', widget.store.netWorth),
            ],
          ),
            ...
            child: BarChart(
              BarChartData(
                ...
                barGroups: [
                  BarChartGroupData(
                    x: 0,
                    barRods: [BarChartRodData(toY: inc, width: 34)],
                  ),
                  BarChartGroupData(
                    x: 1,
                    barRods: [BarChartRodData(toY: exp, width: 34)],
                  ),
                ],
                  ...
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (v, m) => Padding(
                        ...
                        child: Text(v == 0 ? 'Ingresos' : 'Gastos'),
                      ),
                    ),
                  ),
                ),
              ...
              .map(
                (e) => ListTile(
                  ...
                  subtitle: LinearProgressIndicator(
                    value: exp == 0
                        ? 0.0
                        : (e.value / exp).clamp(0.0, 1.0).toDouble(),
                  ),
                  trailing: Text(
  ...
  Widget _m(String t, double v) => SizedBox(
    width: 190,
    child: Card(
      child: Padding(
