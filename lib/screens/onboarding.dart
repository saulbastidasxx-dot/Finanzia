...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  void initState() {
    super.initState();
    PreferencesService().onboardingDone.then((v) {
      if (mounted)
        setState(() {
          done = v;
          loading = false;
  ...
  Widget build(BuildContext c) {
    ...
    final items = [
      (
        ...
        'Organiza cuentas, tarjetas, gastos e ingresos con una vista clara.',
      ),
      (
        Icons.track_changes_outlined,
        'Planifica con intención',
        'Crea presupuestos, metas y recordatorios para mantener el rumbo.',
      ),
      (
        Icons.lock_outline,
        'Privado y sincronizado',
        'Protege Finanzia con PIN o biometría y sincroniza tus dispositivos.',
      ),
    ];
    return Scaffold(
      body: SafeArea(
                  ...
                  FilledButton(
                    onPressed: () async {
                      if (index < 2) {
                        ...
                        await PreferencesService().finishOnboarding();
