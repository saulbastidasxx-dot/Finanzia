...
class _AppLockGateState extends State<AppLockGate> with WidgetsBindingObserver {
  ...
  Future<void> _unlock()async{
    final result=await security.verifyPin(pin.text);
    if(!mounted)return;
    switch(result){
      case PinCheckResult.success: pin.clear();setState((){locked=false;error=null;});return;
      case PinCheckResult.invalid: setState(()=>error='PIN incorrecto');return;
      case PinCheckResult.locked:
        final d=await security.remainingLockout();if(!mounted)return;
        final seconds=d.inSeconds.clamp(1,9999);setState(()=>error='Demasiados intentos. Intenta de nuevo en $seconds s.');return;
    }
  }
  Future<void> _bio()async{if(await security.authenticateBiometric()&&mounted)setState((){locked=false;error=null;});}
