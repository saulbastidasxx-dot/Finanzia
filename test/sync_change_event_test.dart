...
void main() {
  test('change event json round trip', () {
    ...
    expect(x.key, 'transaction:tx');
    expect(x.changedAt, at);
    expect(x.payload?['amount'], 42.5);
    expect(x.synced, isFalse);
  });
  test('copyWith preserves event identity', () {
    ...
    final x = e.copyWith(synced: true);
    expect(x.synced, isTrue);
    expect(x.entityId, 'a');
    expect(x.action, 'delete');
  });
}
